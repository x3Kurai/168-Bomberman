...
public class Bomb2 : MonoBehaviour {
    ...
    void Start () {
        Invoke ("Explode", 3f);
        player = GameObject.Find ("BomberMan_2");
        range = player.GetComponent<PlayerController> ().range;
    }

    void Update() {
        range = player.GetComponent<PlayerController2> ().range;
    }
    ...
    void Explode() {
        ...
        if (player) {
            player.GetComponent<PlayerController2> ().bombs++;
            player.GetComponent<PlayerController2> ().SetBombText ();
        }
    }
